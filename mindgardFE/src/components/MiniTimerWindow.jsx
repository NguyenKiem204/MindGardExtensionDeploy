...
export default function MiniTimerWindow() {
  ...
  const toggleTimer = async () => {
    ...
    if (isFocusMode && window.chrome?.storage?.local) {
      window.chrome.storage.local.set({ focusSessionActive: newRunning });
    }
  };
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center px-6 py-12">
        ...
        <div className="flex items-center gap-6 justify-center mb-8">
          <button
            ...
            className={`text-2xl font-medium transition-colors ${isFocusMode ? 'text-white drop-shadow-lg' : 'text-white/50'
              }`}
          >
            FOCUS
          </button>
        ...
        <button
          onClick={toggleTimer}
          className={`w-16 h-16 rounded-full text-white transition-all shadow-2xl flex items-center justify-center ${isRunning
