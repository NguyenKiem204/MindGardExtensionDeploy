...
export default function FocusModeModal({ isOpen, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Restore the saved focusMode tab
        if (st.focusMode === 'ai') setModeTab('ai');
        else setModeTab('manual');
  ...
  const saveAll = async () => {
    try {
      const saveData = {
        focusMode: modeTab, // 'manual' or 'ai'
            ...
            {!isPlusUser ? (
              ...
              <div className="space-y-5">
                ...
                <div className="flex items-start gap-3 bg-blue-500/10 border border-blue-500/20 rounded-lg p-3">
                  ...
                  <div>
                    <h4 className="text-white font-medium text-sm">Chế độ AI Focus</h4>
                    <p className="text-white/60 text-xs mt-1">
                      AI sẽ tự động phân tích nội dung video YouTube bạn đang xem. Nếu video không liên quan đến chủ đề học tập, bạn sẽ nhận được cảnh báo.
                    </p>
                  </div>
                </div>
                ...
                {/* Focus Topic Input */}
                <div>
                  <label className="text-white/80 text-sm font-medium block mb-2">📚 Chủ đề học tập hôm nay</label>
                  <input
                    value={focusTopic}
                ...
                <div className="bg-white/5 border border-white/10 rounded-lg p-3 space-y-2">
                  <h4 className="text-white/80 text-xs font-semibold uppercase tracking-wider">Cách hoạt động</h4>
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-2 text-white/60 text-xs">
                      <span className="w-5 h-5 rounded-full bg-green-500/20 text-green-400 flex items-center justify-center text-[10px] font-bold">1</span>
                      <span>Bạn nhập chủ đề học tập và bật Focus Mode</span>
                    </div>
                    <div className="flex items-center gap-2 text-white/60 text-xs">
                      <span className="w-5 h-5 rounded-full bg-green-500/20 text-green-400 flex items-center justify-center text-[10px] font-bold">2</span>
                      <span>Khi bạn xem YouTube, AI phân tích video có liên quan không</span>
                    </div>
                    <div className="flex items-center gap-2 text-white/60 text-xs">
                      ...
                      <span>Video không liên quan → Cảnh báo với 2 lựa chọn</span>
                    </div>
                  </div>
                </div>
              </div>
